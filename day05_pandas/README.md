...
```
day_5.ipynb       # Main notebook
testcsv           # Input CSV file (required to run cell 2)
output_test.csv   # Output file generated by the notebook
```

> **Note:** Make sure a file named `testcsv` exists in the same directory as the notebook before running. The notebook reads it in the second cell.
...
2. Place your `testcsv` file in the same directory.
3. Open the notebook:
   ```bash
